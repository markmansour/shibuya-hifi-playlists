...
cat "$llm_input_file" | llm -m claude-4-sonnet > "$llm_output_file.new"

echo "\nProcessing output to extract CSV..."
if grep -q "date,artist,album,year" "$llm_output_file.new"; then
    grep -A 1000 "date,artist,album,year" "$llm_output_file.new" > "$csvfile.new"
    echo "CSV data extracted to $csvfile.new"
else
...
parse_with_llm() {
    ...
    system_prompt="You are parsing a file containing a listening calendar of music albums. I want to pull data out of the text I provide which was converted from html.  identify the date the album will be played on, the artist, the album, and the year of release of the album.  Sort by the date the albums will be played, and not the year they were released. Remove any duplicates. Remove \"Deep Listening\" from any prefixes. Only respond with the properly formatted CSV data and nothing else.  Make sure you include the CVS header line.
...
\"Wednesday Jan 1, 2025 5:00 AM\",\"Daft Punk\",\"Random Access Memories\",2013"
    ...
    log_message "Saved system prompt to $llm_input_file"

    # Run llm with Claude 4 Sonnet and capture its response
    log_message "Running llm with claude-4-sonnet..."
    if cat "$textfile" | llm -m claude-4-sonnet -s "$system_prompt" > "$llm_output_file"; then
    ...
    if grep -q "date,artist,album,year" "$csvfile"; then
        ...
        if [[ "$line_count" -lt 2 ]]; then
            log_message "WARNING: CSV appears to have only a header without data."
        else
